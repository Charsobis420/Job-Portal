import { Indicator, Menu, rem, Notification, Stack } from "@mantine/core";
...
const NotiMenu = () => {
    ...
    useEffect(() => {
        getNotifications(user?.profileId).then((res) => {
    ...
    return <Menu shadow="md" width={400} opened={opened} onChange={setOpened}>
        ...
        <Menu.Dropdown onChange={() => setOpened(true)}>
            <div className="flex flex-col gap-1">
                {
                    notifications.map((noti: any, index: number) => <Notification onClick={() => { navigate(noti.route); unread(index); setOpened(false); }} key={index} className="hover:bg-mine-shaft-900 cursor-pointer" onClose={() => unread(index)} icon={<IconCheck  style={{ width: rem(20), height: rem(20) }} />} color="teal" title={noti.action} mt="md">
                {noti.message}
            </Notification> )
                }
                {
